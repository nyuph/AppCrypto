...
if __name__ == '__main__':
    ...
    # Generate all possible 256 known pairs
    known_pairs = []
    for pairnum in range(0, 256):
        plaintext_bin = bin(pairnum)
        known_plaintext = plaintext_bin[2:]
        if len(known_plaintext) < 8:
            known_plaintext = "0" * (8 - len(known_plaintext)) + known_plaintext
        known_ciphertext = encryption(known_plaintext, generated_key)
        pair = {'plaintext': known_plaintext, 'ciphertext': known_ciphertext}   
